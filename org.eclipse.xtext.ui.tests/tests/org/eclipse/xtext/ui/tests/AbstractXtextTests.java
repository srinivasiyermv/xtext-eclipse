 ...
 * @since 2.3
 */
public abstract class AbstractXtextTests implements ResourceLoadHelper {
	...
	protected void with(Module ... modules) throws Exception {
		assertTrue(canCreateInjector, "super.setUp() has to be called before any injector is instantiated");
	...
	public final XtextResource getResourceAndExpect(InputStream in, URI uri, int expectedErrors) throws Exception {
		...
		if (expectedErrors != UNKNOWN_EXPECTATION) {
			if (expectedErrors == EXPECT_ERRORS)
				assertFalse(resource.getErrors().isEmpty(), Joiner.on('\n').join(resource.getErrors()));
			else
				assertEquals(expectedErrors, resource.getErrors().size(), Joiner.on('\n').join(resource.getErrors()));
		}
